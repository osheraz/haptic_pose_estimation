...
from geometry_msgs.msg import Point, PointStamped, PoseStamped, Quaternion, TransformStamped, Vector3, WrenchStamped, Wrench
...
class hand_control():
    ...
    finger_initial_offset = np.array([0., 0., 0.])
    finger_opening_position = np.array([0.0, 0.0, 0.])
    finger_closing_position = np.array([0., 0., 0.])
    finger_move_offset = np.array([0.01, 0.01, 0.01])

    # Gripper properties
    gripper_status = 'open'
    closed_load = np.array(20.)
    gripper_pos = np.array([0., 0., 0.])
    gripper_load = np.array([0., 0., 0.])
    gripper_temperature = np.array([0., 0., 0.])
    ...
    base_theta = 0  # w.r.t OptiTrack world
    calib_angle = 0  # to start from zero
    base_height = -1.0e3
    obj_pos = [0., 0., 0.]
    obj_height = -1.0e3
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/robotiq_force_torque_wrench_filtered', WrenchStamped, self.callbackTorque)
    ...
    def callbackTorque(self, msg):
        self.gripper_torque = np.concatenate([np.array(msg.force), np.array(msg.torque)])
        print(self.gripper_torque)
    ...
    def CloseGripper(self, msg):
        ...
        for i in range(200):
            ...
            desired = self.gripper_pos + np.array([a for a in self.finger_move_offset])  # slow close
    ...
    def RandomCloseGripper(self, msg):
        ...
        # CHECK OVERHEAT
        count = 0
        delayed_finger = np.random.choice(3)
    ...
    def MoveGripper(self, msg):
        ...
        # msg.angles = [dq1, dq2, dq3], where dq1 and dq2 can be equal to 0 (no move), 1,-1 (increase or decrease angles by finger_move_offset)
        f = 2 #2 # 50.0
    ...
    def GetObservation(self, msg):
        obs = np.concatenate((self.obj_pos, # 3                        0,12
                              # np.expand_dims(self.rot_angle, 0), #     2
                              self.gripper_pos, # 3                    3,45
                              self.gripper_load, # 3                   5,67
                               ))  # 2                 7,8
        return {'state': obs}
